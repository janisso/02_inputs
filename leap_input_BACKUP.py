...
class REG():
    ...
    def doReg(self,q,u_phase,q1,amp,stop_all):
        while True:
            if not q.empty():
                start = time.time()
                yo = q.get()
...
def getSamples(vel,playbackFlag,stop_all):
...
def runLeap(q,vel,u_phase,unravelTime,stop_all):
    #controller = Leap.Controller()
    #uPhase = 0
    #vel = 0
    avg_vel = 0
...
def printPhase(q1,vel,unravelTime,u_phase,playbackFlag,stop_all):
...
def playMIDI(unravelTime,amp,stop_all):
    ...
    for msg in mid:
        ...
        if not msg.is_meta:
            ...
            s_times[msg_count]=[msg_count,all_time]
            msg_count += 1
...
def doIt():
    ...
    p0 = multiprocessing.Process(target=playMIDI,args=(unravelTime,amp,stop_all))
    p1 = multiprocessing.Process(target=runLeap,args=(q,vel,u_phase,unravelTime,stop_all))
    p2 = multiprocessing.Process(target=r.doReg,args=(q,u_phase,q1,amp,stop_all))
    p3 = multiprocessing.Process(target=getSamples,args=(vel,playbackFlag,stop_all))
    p4 = multiprocessing.Process(target=printPhase,args=(q1,vel,unravelTime,u_phase,playbackFlag,stop_all))
